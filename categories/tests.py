...
class CategoryTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.category1 = Category.objects.create(owner=self.user1, name="Work")
        self.category2 = Category.objects.create(owner=self.user1, name="Personal")
        self.category3 = Category.objects.create(owner=self.user2, name="Finance")
    ...
    def test_update_own_category(self):
        ...
        '''
        response = self.client.put(f'/categories/{self.category1.id}/', {"name": "Updated Work"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_category_invalid_data(self):
        ...
        '''
        response = self.client.put(f'/categories/{self.category1.id}/', {"name": ""})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_update_category_not_owned(self):
        ...
        '''
        response = self.client.put(f'/categories/{self.category3.id}/', {"name": "Unauthorized Update"})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TaskCategoryTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.task1 = Task.objects.create(
            owner=self.user1, title="Task 1", category=self.category1, due_date=date(2024, 2, 10)
        )
        self.task2 = Task.objects.create(
            owner=self.user1, title="Task 2", category=self.category2, due_date=date(2024, 3, 15)
        )
    ...
    def test_delete_category_with_tasks(self):
        '''
        If a category is deleted, tasks linked to it should be handled correctly.
        '''
        response = self.client.delete(f'/categories/{self.category1.id}/')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

        # Check if task1 still exists and has its category removed (set to null)
        self.task1.refresh_from_db()
        self.assertIsNone(self.task1.category)  # Ensure category was removed but task still exists
